...
bot2 = discord.Bot(command_prefix="/")  # Instantiate the Bot2bot2 class from the discord module
...
@bot2.slash_command(description="Add 20% to a value which is good for calculating clean cash to dirty")
async def dirty(ctx, value: float):
    new_value = value * 1.2
    await ctx.respond(f"The new value is {new_value:.2f}")

@bot2.slash_command(description="Remove 20% to a value which is good for calculating dirty to clean cash")
async def clean(ctx, value: float):
    new_value = value / 1.2
    await ctx.respond(f"The new value is {new_value:.2f}")

#maths
@bot2.slash_command(description="adds two values")
async def sum(ctx, a: int, b: int):
  await ctx.respond(a + b)

@bot2.slash_command(description="multiplys two values")
async def multiply(ctx, a: int, b: int):
  await ctx.respond(a * b)

@bot2.slash_command(description="minuses two values")
async def minus(ctx, a: int, b: int):
  await ctx.respond(a - b)
  
@bot2.slash_command(description="divides two values")
async def divide(ctx, a: int, b: int):
  await ctx.respond(a / b)

#purge
@bot2.slash_command(pass_context=True, descirption="Purge an amount of messages")
...
@bot2.slash_command(descirption="The commands that we have")
async def help(ctx):
    embed = discord.Embed(
    ...
    embed.add_field(name="/ping", value="Sends the bot2's latency", inline=True)
    ...
    embed.set_footer(text="Bot2bot2 created by Camerinio.") # footers can have icons too
...
bot2.run("MTEyMDA5ODAzNzI4MTMzMzM4OA.GOsraR.oNlbITGHy3PZ40pkjaEPY8fYnaFX_9jtjbphqU")